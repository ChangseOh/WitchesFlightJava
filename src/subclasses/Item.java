...
public class Item {
	...
	public int process(int myX, int myY, Rectangle myRect, int magnet){
		int ret = NOPROCESS;
		
		cnt++;
		
		if(magnet>0 && manager.getRange(x, y, myX, myY)<30000 * magnet){
			//자석이 동작하고 있을 때
			
			if(x + rect.width/2>myX + myRect.width/2)
				x-=20;
			else if(x + rect.width/2<myX + myRect.width/2)
				x+=20;

			if(y + rect.height/2>myY + myRect.height/2)
				y-=20;
			else if(y + rect.height/2<myY + myRect.height/2)
				y+=20;
